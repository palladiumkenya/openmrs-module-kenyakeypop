...
/**
 * Controller for kpClient Allergies and chronic illnesses summary panel
 */
public class ClientAllergiesAndChronicIllnessesFragmentController {
	
	public void controller(@FragmentParam(value = "kpClient") Patient patient, @SpringBean KenyaUiUtils kenyaUi,
		...
		List<Form> formsCollectingAllergies = Arrays.asList(
		    Context.getFormService().getFormByUuid("47814d87-2e53-45b1-8d05-ac2e944db64c"), Context.getFormService()
		            .getFormByUuid("22c68f86-bbf0-49ba-b2d1-23fa7ccf0259"));
		
		List<Encounter> encounters = Context.getEncounterService().getEncounters(patient, null, null, null,
		    formsCollectingAllergies, null, null, false);
		...
		List<Obs> allergies = obsService.getObservations(patients, encounters, question, null, null, null, null, null, null,
		    null, null, false);
		...
		for (Obs obs : allergies) {
			allergyCounter++;
			String allergen = obs.getValueCoded().equals(noConcept) ? "None" : allergyCounter + "."
			        + obs.getValueCoded().getName().getName();
			allergyList.add(SimpleObject.create("allergen", allergen));
		}
		...
		for (Obs o : chronicIllnesses) {
			if (o.getValueCoded() != null && o.getValueCoded() == noConcept) {
				...
				illnessList.add(SimpleObject.create("illness", illness));
			}
		}
		
		model.addAttribute("kpClient", patient);
