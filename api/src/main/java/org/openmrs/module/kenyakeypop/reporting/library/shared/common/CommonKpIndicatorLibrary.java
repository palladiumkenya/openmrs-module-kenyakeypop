 ...
 */
package org.openmrs.module.kenyakeypop.reporting.library.shared.common;
 ...
 */
@Component
public class CommonKpIndicatorLibrary {
	
	@Autowired
	private CommonKpCohortLibrary commonCohorts;
	...
	public CohortIndicator enrolled(Program program) {
		return cohortIndicator("new patients enrolled in " + program.getName() + " including transfers", ReportUtils.map(
		    commonCohorts.enrolledExcludingTransfers(program), "onOrAfter=${startDate},onOrBefore=${endDate}"));
	}
	...
	public CohortIndicator enrolledExcludingTransfers(Program program) {
		return cohortIndicator("new patients enrolled in " + program.getName() + " excluding transfers", ReportUtils.map(
		    commonCohorts.enrolledExcludingTransfers(program), "onOrAfter=${startDate},onOrBefore=${endDate}"));
	}
