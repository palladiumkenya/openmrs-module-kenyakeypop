...
public class EligibleForNonClinicalServices extends BaseEmrCalculation {
	
	protected static final Log log = LogFactory.getLog(EligibleForNonClinicalServices.class);
			...
			if (activeInKpProgram.contains(ptId)) {
				...
				if (programs.size() > 0) {
					...
					if (lastClinicalEnrolmentEnc == null
					        || lastKpenrollmentDate.after(lastClinicalEnrolmentEnc.getEncounterDatetime())) {
						eligible = true;
					}
				}
			}
