...
public class MonthlyReportCohortLibrary {
	...
	public CohortDefinition contactAll(String kpType) {
		...
		String sqlQuery = "select p.client_id from kenyaemr_etl.etl_kp_peer_calendar p inner join kenyaemr_etl.etl_kp_contact c on p.client_id = c.client_id where date(c.visit_date) between date(:startDate) and date(:endDate) and c.key_population_type = "
	...
	public CohortDefinition everEnroll(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select e.client_id from kenyaemr_etl.etl_kp_client_enrollment e inner join kenyaemr_etl.etl_kp_contact c on e.client_id = c.client_id\n"
	...
	public CohortDefinition contactNew(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select p.client_id from kenyaemr_etl.etl_kp_peer_calendar p inner join kenyaemr_etl.etl_kp_contact c on c.client_id = p.client_id\n"
	...
	public CohortDefinition contactHCW(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c join kenyaemr_etl.etl_kp_client_enrollment e on c.client_id = e.client_id and e.voided = 0\n"
		        + "left join (select v.client_id from kenyaemr_etl.etl_kp_clinical_visit v where v.voided = 0 and v.date_created between date(:startDate) and date(:endDate) group by v.client_id ) v on c.client_id=v.client_id\n"
		        + "left join (select p.client_id from kenyaemr_etl.etl_kp_peer_calendar p where p.voided = 0 and p.date_created between date(:startDate) and date(:endDate) group by p.client_id ) p on c.client_id=p.client_id\n"
	...
	public CohortDefinition kpPrev(String kpType) {
		...
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c\n"
		        ...
		        + "    left join (select v.client_id, min(v.visit_date) as first_clinical_visit_date from kenyaemr_etl.etl_kp_clinical_visit v group by v.client_id)v on c.client_id = v.client_id\n"
		        + "    left join (select p.client_id, min(p.visit_date) as first_peer_enc from kenyaemr_etl.etl_kp_peer_calendar p group by p.client_id)p on c.client_id = p.client_id\n"
	...
	public CohortDefinition kpCurr(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c join kenyaemr_etl.etl_kp_client_enrollment e on c.client_id = e.client_id and e.voided = 0\n"
		        + "join (select p.client_id from kenyaemr_etl.etl_kp_peer_calendar p where p.voided = 0 group by p.client_id having max(p.visit_date) between date_sub(date(:endDate), INTERVAL 3 MONTH ) and date(:endDate)) p on c.client_id=p.client_id\n"
	...
	public CohortDefinition assistedSelfTested(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join kenyaemr_etl.etl_kp_clinical_visit v on c.client_id = v.client_id\n"
		        + "where c.key_population_type = "
		        + kpType
		        + " group by c.client_id\n"
	...
	public CohortDefinition htsTestedNegative(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join kenyaemr_etl.etl_hts_test t on c.client_id = t.patient_id\n"
		        + "where c.key_population_type = "
		        + kpType
	...
	public CohortDefinition newOnARTKP(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c left join\n"
		        + "(select v.client_id,mid(max(concat(v.visit_date,v.initiated_art_this_month)),11) as started_art_this_month from kenyaemr_etl.etl_kp_clinical_visit v where v.voided = 0 group by v.client_id)v on c.client_id = v.client_id\n"
	...
	public CohortDefinition currOnARTKP(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c\n"
		        + "left join (select v.client_id from kenyaemr_etl.etl_kp_clinical_visit v where v.voided = 0 group by v.client_id having mid(max(concat(v.visit_date,v.active_art)),11)='Yes')v on c.client_id = v.client_id\n"
	...
	public CohortDefinition screenedForSTI(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join kenyaemr_etl.etl_kp_clinical_visit v on c.client_id = v.client_id where c.key_population_type = "
		        + kpType
	...
	public CohortDefinition receivedGbvClinicalCare(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join kenyaemr_etl.etl_gbv_screening_action v on c.client_id = v.patient_id\n"
	...
	public CohortDefinition eligibleForRetest(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join\n"
	...
	public CohortDefinition kplhivInitiatedARTOtherPEPFAR(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join\n"
		        + "(select t.client_id from kenyaemr_etl.etl_kp_treatment_verification t group by t.client_id having mid(max(concat(t.visit_date,t.date_initiated_art)),11) between date(:startDate) and date(:endDate)\n"
	...
	public CohortDefinition kplhivCurrentOnARTOtherPEPFAR(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join\n"
		        + "(select t.client_id from kenyaemr_etl.etl_kp_treatment_verification t group by t.client_id having mid(max(concat(t.visit_date,t.date_initiated_art)),11) <= date(:endDate)\n"
		        + "and mid(max(concat(t.visit_date,t.is_pepfar_site)),11)='Yes' )t on c.client_id = t.client_id\n"
		        + "where c.voided = 0 and c.key_population_type = " + kpType + " group by c.client_id;";
	...
	public CohortDefinition kplhivLTFURecently(String kpType) {
		...
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c\n"
		        ...
		        + "left join (select v.client_id, max(v.visit_date),mid(max(concat(v.visit_date,v.appointment_date)),11) as appointment_date from kenyaemr_etl.etl_kp_clinical_visit v )v\n"
		        + "on c.client_id = v.client_id\n"
		        + "left join (select t.client_id, max(t.visit_date) as latest_track, mid(max(concat(t.visit_date,t.status_in_program)),11) as status_in_program from kenyaemr_etl.etl_kp_peer_tracking t) t on c.client_id = t.client_id\n"
		        + "left join (select d.patient_id, date(max(d.visit_date)) latest_visit from kenyaemr_etl.etl_patient_program_discontinuation d where d.program_name='KP' group by d.patient_id) d on c.client_id = d.patient_id\n"
	...
	public CohortDefinition kplhivSuppressedVlArtOtherPEPFARSite(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join\n"
		        + "(select t.client_id from kenyaemr_etl.etl_kp_treatment_verification t group by t.client_id having mid(max(concat(t.visit_date,t.is_pepfar_site)),11)='Yes'\n"
	...
	public CohortDefinition kplhivSuppressedVlArtNonPEPFARSite(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join\n"
		        + "(select t.client_id from kenyaemr_etl.etl_kp_treatment_verification t group by t.client_id having mid(max(concat(t.visit_date,t.is_pepfar_site)),11)='No'\n"
		        + "and mid(max(concat(t.visit_date,t.vl_test_date)),11) between date(:startDate) and date(:endDate) and mid(max(concat(t.visit_date,t.viral_load)),11) <1000 or UPPER(mid(max(concat(t.visit_date,t.viral_load)),11))='LDL') v\n"
		        + "on c.client_id = v.client_id where c.key_population_type = " + kpType
		        + " and c.voided = 0 group by c.client_id;";
	...
	public CohortDefinition kpOnMultiMonthART(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c left join\n"
		        + "(select t.client_id, max(t.visit_date) as visit_date from kenyaemr_etl.etl_kp_treatment_verification t where t.on_diff_care = 'Yes' group by t.client_id) t on c.client_id = t.client_id\n"
		        + "left join\n"
	...
	public CohortDefinition kpEnrolledInARTSupportGroup(String kpType) {
		...
		SqlCohortDefinition cd = new SqlCohortDefinition();
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join kenyaemr_etl.etl_kp_treatment_verification v on c.client_id = v.client_id\n"
	...
	public CohortDefinition pnsKnownPositiveAtEntry(String kpType) {
		...
		String sqlQuery = "select c.client_id from kenyaemr_etl.etl_kp_contact c inner join\n"
		        ...
		        + " where c.relationship_type in(971, 972, 1528, 162221, 163565, 970, 5617)\n"
		        + "and c.voided = 0 and date(c.date_created) between date(:startDate) and date(:endDate)) pns on c.client_id = pns.patient_id\n"
		        + "inner join kenyaemr_etl.etl_kp_client_enrollment e on pns.patient_id = c.client_id\n"
	...
	public CohortDefinition txPvlsEligibleDice(String kpType) {
		...
		String sqlQuery = "select client_id\n"
		        ...
		        + "from kenyaemr_etl.etl_kp_contact c\n"
	...
	public CohortDefinition txPvlsEligibleVerifyPEPFAR(String kpType) {
		...
		String sqlQuery = "select client_id\n"
		        ...
		        + " and c.voided = 0\n"
		        + "group by c.client_id) a\n"
		        + "inner join kenyaemr_etl.etl_kp_treatment_verification l on l.client_id = a.client_id\n"
		        + "group by a.client_id) o\n"
		        + ") e  where\n"
		        + "(\n"
	...
	public CohortDefinition txPvlsEligibleDoneVerifyPEPFAR(String kpType) {
		...
		String sqlQuery = "select client_id\n"
		        ...
		        + " from kenyaemr_etl.etl_kp_treatment_verification l group by l.client_id having max(vl_test_date) between date_sub(:endDate, interval 12 month) and date(:endDate) )l on l.client_id = a.client_id\n"
		        + "group by a.client_id) o\n"
		        + ") e  where\n"
		        + "(\n"
		        + "e.lastVL is not null  and (e.lastVL < 1000 or e.lastVL=1302) and (timestampdiff(YEAR,e.dob,:endDate))<25 and  timestampdiff(MONTH,e.lastVLDate, :endDate) >= 6\n"
