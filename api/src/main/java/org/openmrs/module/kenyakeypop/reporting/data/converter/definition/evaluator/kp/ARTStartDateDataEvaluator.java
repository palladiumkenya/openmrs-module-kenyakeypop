...
import org.openmrs.module.kenyakeypop.reporting.data.converter.definition.kp.ARTStartDateDataDefinition;
...
public class ARTStartDateDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "SELECT  t1.client_id, COALESCE(t1.date_initiated_art, t2.date_initiated_art) AS date_initiated_art\n"
		        + "FROM (\n"
		        + "  SELECT v.client_id, MID(MAX(CONCAT(v.visit_date, v.date_initiated_art)), 11) AS date_initiated_art\n"
		        + "  FROM kenyaemr_etl.etl_treatment_verification v\n"
		        + "  WHERE DATE(v.visit_date) BETWEEN DATE(:startDate) AND DATE(:endDate)) AS t1\n" + "LEFT JOIN (\n"
		        + "  SELECT d.patient_id, MID(MAX(CONCAT(d.visit_date, d.date_initiated_art)), 11) AS date_initiated_art\n"
