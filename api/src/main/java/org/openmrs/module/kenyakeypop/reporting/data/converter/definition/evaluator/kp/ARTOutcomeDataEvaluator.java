...
public class ARTOutcomeDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select r.patient_id,if( r.dead= 0,(coalesce((case when timestampdiff(Month,max(date(v.visit_date)),date(:endDate))<=3  then \"A\" when timestampdiff(Month,max(date(v.visit_date)),date(:endDate)) between 4 and 9 then \"DT\"\n"
		        ...
		        + "when timestampdiff(Month,max(date(p.visit_date)),date(:endDate)) > 9 then \"LTFU\" else \"\" end))),\"D\") as status_in_program from kenyaemr_etl.etl_patient_demographics r\n"
