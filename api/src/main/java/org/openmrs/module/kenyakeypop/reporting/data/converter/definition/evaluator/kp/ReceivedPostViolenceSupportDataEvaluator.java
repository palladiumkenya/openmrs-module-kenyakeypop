...
public class ReceivedPostViolenceSupportDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select v.client_id, (case mid(max(concat(v.visit_date,v.violence_treated)),11)\n"
		        + "                         when 'Supported' then 'Y' else '' end )as violence_treated from kenyaemr_etl.etl_clinical_visit v\n"
		        + "where v.violence_treated = 'Supported' and date(v.visit_date) between date(:startDate) and date(:endDate)\n"
